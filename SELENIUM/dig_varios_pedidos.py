...
def digitar_pedidos_online(file):
    ...
    for cliente in list_cliente:
        ...
        print(f'ID   Codigo        \t\tQTD\t\tVALOR  \t\t  ORDEM DE COMPRA')
        sleep(5)
        ...
        for item in lista_pedidos2:
            ...
            if str(item[0])== cliente:
                ...
                #login_bling.start.iniciar()
                    ...
                    sleep(1)
                    print(f'{str(cont)}\t{str(item[1]):14} \t\t{str(item[2]):>3} \t\t{str(item[3]):>7} \t\t{str(item[4]):10}')
                    oc_ant = str(item[4]) 
                    #print(f'Caso Igual: {oc} - {oc_ant}')
                    cont+=1
                else:
                    print(f'Valor total.......................{total:5.2f}')
                    start.digitar_info_ped(str(item[4]))
                    sleep(2)
                    start.salvar_pedido()
                    sleep(2)
